...
## Unit Tests — Table-Driven with Mockery

All unit tests use table-driven style with `t.Run`. Mocks are generated by **mockery** — never write mocks by hand.

### Generating mocks

```bash
make generate-mocks   # runs: mockery --config mockery.yaml
```

Register every interface that needs mocking in `mockery.yaml`:

```yaml
# mockery.yaml (root of backend/)
with-expecter: true
dir: "{{.InterfaceDirRelative}}/mocks"
outpkg: mocks
filename: "{{.InterfaceName}}_gen.go"
resolve-type-alias: false
disable-version-string: true
issue-845-fix: true
packages:
  github.com/drakoRRR/.../internal/services/experiment:
    interfaces:
      Storage:
  github.com/drakoRRR/.../internal/handler/http/api/experiment:
    interfaces:
      Service:
```

Generated files land in `<package>/mocks/<InterfaceName>_gen.go`. **Never edit them.**
...
func TestService_StartExperiment(t *testing.T) {
    tests := []struct {
        name      string
        setup     func(*mocks.MockStorage)
        wantErr   error
    }{
        {
            name: "transitions draft to running",
            setup: func(s *mocks.MockStorage) {
                s.EXPECT().GetByID(mock.Anything, mock.Anything).Return(draftExp, nil)
                s.EXPECT().Update(mock.Anything, mock.Anything).
                    RunAndReturn(func(_ context.Context, e domain.Experiment) (domain.Experiment, error) {
                        return e, nil
                    })
            },
        },
        {
            ...
            setup: func(s *mocks.MockStorage) {
                s.EXPECT().GetByID(mock.Anything, mock.Anything).Return(runningExp, nil)
            },
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            storage := mocks.NewMockStorage(t)  // auto-registers AssertExpectations on cleanup
            tt.setup(storage)
            svc := New(storage)

            err := svc.StartExperiment(context.Background(), uuid.New())
...
- Use `mocks.NewMockXxx(t)` — it auto-registers `AssertExpectations` via `t.Cleanup`
- Use `EXPECT()` typed expecter API — never `mock.On("Method", ...)` string-based form
- Use `mock.Anything` for context and arguments you don't need to assert on specifically
